...
const { Type }      = require('demciak-distribution-types')
...
let fatalStartupErrorDisplayable = null
...
async function bootMainUIFromDistribution(data = null){
    try {
        const distroData = data != null ? data : await DistroAPI.getDistribution()
        syncModConfigurations(distroData)
        ensureJavaSettings(distroData)
        await showMainUI(distroData)
    } catch(err) {
        console.error('Fatal startup error while preparing UI.', err)
        fatalStartupError = true
        fatalStartupErrorDisplayable = {
            title: 'Błąd startu UI',
...
async function showMainUI(data){
    ...
    if(!settingsPrepared){
        try {
            ...
            console.error('Settings initialization failed during startup.', err)
        }
    }
    updateSelectedServer(data.getServerById(ConfigManager.getSelectedServer()))
    populateLandingServerList(data)
    refreshServerStatus()
    setTimeout(() => {
...
function showFatalStartupError(){
    setTimeout(() => {
        $('#loadingContainer').fadeOut(250, () => {
            ...
            setOverlayContent(
                fatalStartupErrorDisplayable?.title || Lang.queryJS('uibinder.startup.fatalErrorTitle'),
                fatalStartupErrorDisplayable?.message || Lang.queryJS('uibinder.startup.fatalErrorMessage'),
    ...
    if (document.readyState === 'interactive' || document.readyState === 'complete'){
        if(rscShouldLoad){
            ...
            if(!fatalStartupError){
                await bootMainUIFromDistribution()
            } else {
                showFatalStartupError()
            }
        } 
...
ipcRenderer.on('distributionIndexDone', async (event, res) => {
    if(res) {
        ...
        if(document.readyState === 'interactive' || document.readyState === 'complete'){
            await bootMainUIFromDistribution(data)
        } else {
            rscShouldLoad = true
        }
    } else {
