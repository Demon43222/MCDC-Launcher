...
const { getMojangOS, isLibraryCompatible, mcVersionAtLeast }  = require('demciak-core/common')
...
function isForgeFamilyModType(type){
    return type === Type.ForgeMod || type === Type.NeoForgeMod || type === Type.FabricMod
}
...
class ProcessBuilder {
    ...
    _constructJVMArguments112(mods, tempNativePath){
        ...
        if(process.platform === 'darwin'){
            args.push('-Xdock:name=MCDC.pl Launcher')
    ...
    _constructJVMArguments113(mods, tempNativePath){
        ...
        //args.push('-Dlog4j.configurationFile=D:\\MCDC.pl\\game\\common\\assets\\log_configs\\client-1.12.xml')
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'MCDC-Launcher')
                            break
    ...
    _resolveServerLibraries(mods){
        ...
        for(let mdl of mdls){
            const type = mdl.rawModule.type
            if(type === Type.ForgeHosted || type === Type.Forge || type === Type.Fabric || type === Type.Library){
